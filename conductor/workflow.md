...
```bash
# Install root dependencies (viem, walletconnect)
npm install

# Install test dependencies (jsdom, viem)
...
```bash
# Start local dev server for Web UI (serves index.html on localhost)
npm run dev

# Run CLI directly
node cli.js rollover --help
node cli.js leverage --help

# Run full test suite
npm test --prefix tests && node tests/cli.test.mjs
```

### Before Committing

```bash
# Run all tests (unit, integration, simulation)
npm test --prefix tests && node tests/cli.test.mjs

# Verify no oversized modules
find . -name "*.js" -not -path "*/node_modules/*" -exec wc -l {} + | sort -rn | head -10
```
